...
describe("createGameOfLife", () => {
    ...
    describe("UI", () => {
        ...
        it("draws field", () => {
            (drawField as jest.Mock).mockImplementation((fieldEl: Element, field: number[][]) => {
        ...
        it("redraw field on interaction with it", () => {
            let onCellClick: ((x: number, y: number) => void) | undefined;
            (drawField as jest.Mock).mockImplementation((fieldEl: Element, field: number[][], cellClickHandler: (x: number, y: number) => void) => {
